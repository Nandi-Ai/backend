...
class DatabaseTest(TestCase):
    ...
    def setUp(self):
        ...
        self.dataset_tag = Tag.objects.create(
            name='Test Tag'
        )
        ...
        self.public_dataset.tags.set(Tag.objects.filter(id__in=[x.id for x in [self.dataset_tag]]))
        self.private_dataset_discoverable.tags.set(Tag.objects.filter(id__in=[x.id for x in [self.dataset_tag]]))
        self.private_dataset_not_discoverable.tags.set(Tag.objects.filter(id__in=[x.id for x in [self.dataset_tag]]))
    ...
    def test_private_dataset(self):
        private_dataset_discoverable = Dataset.objects.get(name=self.private_dataset_discoverable.name)
        private_dataset_not_discoverable = Dataset.objects.get(name=self.private_dataset_not_discoverable.name)
