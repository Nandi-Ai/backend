def __create_where_section_positive(field, operator, value):
    dev_express_value = value if isinstance(value, int) or isinstance(value, float) or value is None else value.lower()
    ...
    for data in data_filter:
        if isinstance(data, list):
            if isinstance(data[0], list):
                ...
                if data[0] == '!':
                    data.pop(0)
                    query += __generate_where_sql_query_from_multiple_filters(data, not negative)
                else:
                    query += __generate_where_sql_query_from_single_filter(data, negative)
