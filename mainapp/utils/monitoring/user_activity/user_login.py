...
class UserLogin(EventHandler):
    _EVENT_TYPE = MonitorEvents.EVENT_USER_LOGIN
    ...
    def _validate_data(self):
        if not self.__user:
            raise InvalidEventData("User Login event must have a user")

    def _log_event(self):
        logger.info(
            f"User {self.__user.display_name} from org {self.__user.organization.name} has logged in"
        )
