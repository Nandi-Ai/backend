...
class DatasetRemoveDatasource(EventHandler):
    _EVENT_TYPE = MonitorEvents.EVENT_DATASET_REMOVE_DATASOURCE
    ...
    def _validate_data(self):
        if not self.__datasource:
            raise InvalidEventData(
                "Dataset Remove Datasource event must have a datasource"
            )

    def _log_event(self):
        logger.info(
            f"User {self._user_name} has removed "
