...
class StudyVMStopRequest(EventHandler):
    _EVENT_TYPE = MonitorEvents.EVENT_STUDY_VM_STOP_REQUEST
    ...
    def _validate_data(self):
        if not self.__study or not self.__user:
            raise InvalidEventData(
                "Request Stop Study VM event must have a study and user"
            )

    def _log_event(self):
        logger.info(
            f"User {self.__user.display_name} has requested to stop VM for study "
