...
def create_sql_for_column(column):
    ...
    if (
        column_type_from_athena in numeric_types
        and column_name_as_in_file == column_name_from_athena
    ):
        return f"""
...
def sql_builder_by_columns_types(
    glue_table, columns_types, default_athena_col_names, filter_query
):
    queries = list(
        map(create_sql_for_column, zip(default_athena_col_names, columns_types))
    )
...
def create_default_column_names(column_names):
    return [
        {"Name": name["Name"], "Col_Name": f"col{i}"}
